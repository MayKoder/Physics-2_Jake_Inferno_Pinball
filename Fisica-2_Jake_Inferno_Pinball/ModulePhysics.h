...
public:
	...
	PhysBody* Create_Poly(float, float, int[], int, b2Vec2[], SDL_Rect sec = { 0, 0, 0, 0 }, b2BodyType type = b2BodyType::b2_dynamicBody, 
		SDL_RendererFlip flip = SDL_FLIP_NONE, b2Vec2 offset = {0, 0}, float density = 1.0f, int hit_score = 0, int sound = -1, Animation* hit_animation = nullptr);
	PhysBody* Create_Rectangle_Sensor(SDL_Rect rectangle, float rotation, SDL_Rect sec = { 0, 0, 0, 0 }, Animation* hit_animation = nullptr);
